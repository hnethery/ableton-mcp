...
class AbletonMCP(ControlSurface):
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    self.log_message(
                        "Connection accepted from " + str(address))
    ...
    def _process_command(self, command):
        ...
        try:
            ...
            # Commands that modify Live's state should be scheduled on the main thread
                ...
                def main_thread_task():
                    try:
                        ...
                        self.log_message(
                            "Error in main thread task: " + str(e))
                        self.log_message(traceback.format_exc())
                        response_queue.put(
                ...
                response["result"] = self._get_browser_categories(
                    category_type)
            ...
            # Add the new browser commands
                ...
                response["result"] = self._set_device_parameter(
                ...
                response["result"] = self._set_eq_band(
                    track_index, device_index, band_index, frequency, gain, q, filter_type)
                ...
                response["result"] = self._set_eq_global(
                    track_index, device_index, scale, mode, oversampling)
            elif command_type == "apply_eq_preset":
                track_index = params.get("track_index", 0)
                device_index = params.get("device_index", 0)
                preset_type = params.get("preset_type", "")
                response["result"] = self._apply_eq_preset(
                    track_index, device_index, preset_type)
            elif command_type == "set_track_volume":
                track_index = params.get("track_index", 0)
                value = params.get("value", 0.0)
    ...
    def _create_midi_track(self, index):
        ...
        try:
            ...
            # Get the new track
            new_track_index = len(self._song.tracks) - \
                1 if index == -1 else index
        ...
        try:
            ...
            if path:
                ...
                for i in range(1, len(path_parts)):
                    ...
                    if not found:
                        result["error"] = "Path part '{0}' not found".format(
                            part)
                        return result
    ...
    def _frequency_to_normalized(self, frequency, min_freq=10.0, max_freq=22000.0):
        ...
        if min_freq <= 0:
            raise ValueError(
                "Minimum frequency must be positive for logarithmic scale")
        ...
        try:
            ...
            for param_index, param in enumerate(device.parameters):
                ...
                if param.is_quantized and len(param.value_items) > 1:
                    param_info["value_items"] = [
                        str(item) for item in param.value_items]
                    param_info["value_item_index"] = int(param.value)
                    param_info["value_item"] = str(
                        param.value_items[int(param.value)])
        ...
        try:
            ...
            if parameter_name is not None:
                ...
                if parameter is None:
                    raise ValueError(
                        f"Parameter '{parameter_name}' not found in device '{device.name}'")
                ...
                raise ValueError(
                    "Either parameter_name or parameter_index must be provided")

            # Check if the parameter is enabled
            if not parameter.is_enabled:
                raise ValueError(
                    f"Parameter '{parameter.name}' is not enabled")
            ...
            if parameter.is_quantized and len(parameter.value_items) > 1:
                ...
                if isinstance(value, str):
                    ...
                    if value_index is None:
                        raise ValueError(
                            f"Value '{value}' not found in parameter value items")
                ...
                if value < 0 or value >= len(parameter.value_items):
                    raise ValueError(
                        f"Value index {value} out of range for parameter '{parameter.name}'")
            else:
                # For continuous parameters, ensure value is within range
                if value < parameter.min or value > parameter.max:
                    raise ValueError(
                        f"Value {value} out of range for parameter '{parameter.name}' (min: {parameter.min}, max: {parameter.max})")
            ...
            if filter_type is not None:
                ...
                if isinstance(filter_type, str):
                    ...
                    if filter_index is None:
                        raise ValueError(
                            f"Filter type '{filter_type}' not found")

                    filter_param.value = filter_index
                    results["filter_type"] = str(
                        filter_param.value_items[filter_index])
                else:
                    # Assume filter_type is an index
                    if filter_type < 0 or filter_type >= len(filter_param.value_items):
                        raise ValueError(
                            f"Filter type index {filter_type} out of range")

                    filter_param.value = filter_type
                    results["filter_type"] = str(
                        filter_param.value_items[filter_type])
            ...
            if mode is not None:
                ...
                if hasattr(device, 'global_mode'):
                    ...
                    try:
                        ...
                        mode_str = "Unknown"
                        if device.global_mode == 0:
                            mode_str = "Stereo"
                        elif device.global_mode == 1:
                            mode_str = "L/R"
                        elif device.global_mode == 2:
                            mode_str = "M/S"

                        results["mode"] = mode_str
                    except Exception as e:
                        self.log_message(
                            f"Error setting global_mode property: {e}")
                ...
                if "mode" not in results:
                    ...
                    if mode_param is not None:
                        ...
                        if hasattr(device, 'global_mode'):
                            raise ValueError(
                                "Failed to set Mode: invalid value for global_mode property")
                        else:
            ...
            if oversampling is not None:
                ...
                if hasattr(device, 'oversample'):
                    try:
                        ...
                        self.log_message(
                            f"Error setting oversample property: {e}")
                        pass
                ...
                if "oversampling" not in results:
                    ...
                    if oversampling_param is not None:
                        ...
                        if not hasattr(device, 'oversample'):
                            self.log_message(
                                "Oversampling parameter not found for EQ Eight device")
            ...
            for band_index, settings in preset.items():
                ...
                if settings.get("enabled", False):
                    ...
                    if "freq" in settings:
                        ...
                        normalized_value = self._frequency_to_normalized(
                            frequency)
    ...
    def get_browser_tree(self, category_type="all"):
        ...
        try:
            ...
            browser_attrs = [attr for attr in dir(
                app.browser) if not attr.startswith('_')]
            self.log_message(
                ...
                for attr in browser_attrs:
                    if attr.lower() == root_category:
                        try:
                            ...
                            self.log_message(
                                "Error accessing browser attribute {0}: {1}".format(attr, str(e)))
