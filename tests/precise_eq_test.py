    ...
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            try:
                ...
                print(
                    f"Error decoding JSON response: {response.decode('utf-8')}")
...
def setup_test_environment():
    ...
    if eq_result.get("status") != "success":
        print(
            f"❌ Failed to load EQ Eight. Error: {eq_result.get('message', 'Unknown error')}")
...
def test_precise_frequency_control(track_index, device_index):
    ...
    for band_index in test_bands:
        ...
        if filter_type_param is None:
            print(
                f"❌ Could not find filter type parameter for band {band_number}")
            all_passed = False
            continue
        ...
        if filter_type_index is None:
            print(
                f"❌ Could not find filter type '{filter_type}' for band {band_number}")
            all_passed = False
            continue
        ...
        for freq in test_frequencies:
            ...
            if result.get("status") != "success":
                print(
                    f"❌ Failed to set frequency for band {band_number} to {freq} Hz")
                all_passed = False
                continue
            ...
            if error_pct < 5:  # Allow 5% error due to rounding and approximation
                print(
                    f"✅ Set frequency for band {band_number} to {freq} Hz (actual: {actual_freq} Hz, error: {error_pct:.2f}%)")
            else:
                print(
                    f"⚠️ Set frequency for band {band_number} to {freq} Hz, but actual value is {actual_freq} Hz (error: {error_pct:.2f}%)")
                all_passed = False
...
def test_precise_gain_control(track_index, device_index):
    ...
    for band_index in test_bands:
        ...
        for gain in test_gains:
            ...
            if result.get("status") != "success":
                print(
                    f"❌ Failed to set gain for band {band_number} to {gain} dB")
                all_passed = False
                continue
            ...
            if error < 0.01:  # Allow small error due to floating point precision
                print(
                    f"✅ Set gain for band {band_number} to {gain} dB (actual: {actual_gain} dB)")
            else:
                print(
                    f"⚠️ Set gain for band {band_number} to {gain} dB, but actual value is {actual_gain} dB (error: {error:.2f})")
                all_passed = False
    ...
    for band_index in test_bands:
        ...
        for q in test_q_values:
            ...
            if error_pct < 10:  # Allow 10% error due to approximation in our conversion
                print(
                    f"✅ Set Q for band {band_number} to {q} (actual: {actual_q}, error: {error_pct:.2f}%)")
            else:
                print(
                    f"⚠️ Set Q for band {band_number} to {q}, but actual value is {actual_q} (error: {error_pct:.2f}%)")
                all_passed = False
...
def test_filter_types(track_index, device_index):
    ...
    for band_index in test_bands:
        ...
        for i, filter_type in enumerate(filter_types):
            ...
            if result.get("status") != "success":
                print(
                    f"❌ Failed to set filter type for band {band_number} to {filter_type}")
                all_passed = False
                continue
...
def test_scale_parameter(track_index, device_index):
    ...
    for scale in test_scales:
        ...
        if error < 0.01:  # Allow small error due to floating point precision
            ...
            print(
                f"⚠️ Set Scale to {scale}, but actual value is {actual_scale} (error: {error:.2f})")

    return True


def test_oversampling(track_index, device_index):
    """Test oversampling parameter."""
    print_divider("TESTING OVERSAMPLING")
    ...
    for val in test_values:
        ...
        result = send_command("set_eq_global", {
            "track_index": track_index,
            "device_index": device_index,
            "oversampling": val
        })
        ...
        if result.get("status") != "success":
            print(f"❌ Failed to set oversampling to {val}")
            all_passed = False
            continue

        # Verify result in response
        actual = result.get("result", {}).get(
            "global_parameters", {}).get("oversampling")

        if actual == expected:
            print(f"✅ Set oversampling to {val} (got: {actual})")
        else:
            print(f"⚠️ Set oversampling to {val}, but got {actual}")
            all_passed = False
...
def main():
    ...
    # Run tests
    tests = [
        ("precise_frequency_control", lambda: test_precise_frequency_control(
            track_index, device_index)),
        ("precise_gain_control", lambda: test_precise_gain_control(
            track_index, device_index)),
        ("precise_q_control", lambda: test_precise_q_control(
            track_index, device_index)),
        ...
        ("oversampling", lambda: test_oversampling(track_index, device_index))
    ]
