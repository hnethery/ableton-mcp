...
class AbletonConnection:
    ...
    def receive_full_response(self, sock, buffer_size=8192):
        ...
        try:
            while True:
                try:
                    ...
                    logger.error(
                        f"Socket connection error during receive: {
                            str(e)}")
                    raise
        except Exception as e:
        ...
        if chunks:
            data = b''.join(chunks)
            logger.info(
                f"Returning data after receive completion ({
                    len(data)} bytes)")
            try:
                json.loads(data.decode('utf-8'))
                return data
        ...
        try:
            ...
            if response.get("status") == "error":
                ...
                raise Exception(
                    response.get(
                        "message",
                        "Unknown error from Ableton"))

            return response.get("result", {})
        except socket.timeout:
            logger.error(
                "Socket timeout while waiting for response from Ableton")
            self.sock = None
...
async def server_lifespan(server: FastMCP) -> AsyncIterator[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Could not connect to Ableton on startup: {
                    str(e)}")
...
def get_ableton_connection():
    ...
    if _ableton_connection is None:
        ...
        if _ableton_connection is None:
            logger.error(
                "Failed to connect to Ableton after multiple attempts")
            raise Exception(
...
def create_clip(ctx: Context, track_index: int,
                clip_index: int, length: float = 4.0) -> str:
    """
...
def add_notes_to_clip(
    ...
    try:
        ...
        return f"Added {
            len(notes)} notes to clip at track {track_index}, slot {clip_index}"
    except Exception as e:
...
def load_instrument_or_effect(ctx: Context, track_index: int, uri: str) -> str:
    ...
    try:
        ...
        if result.get("loaded", False):
            ...
            if new_devices:
                return f"Loaded instrument with URI '{uri}' on track {track_index}. New devices: {
                    ', '.join(new_devices)}"
            else:
                devices = result.get("devices_after", [])
                return f"Loaded instrument with URI '{uri}' on track {track_index}. Devices on track: {
                    ', '.join(devices)}"
        else:
...
def get_browser_tree(ctx: Context, category_type: str = "all") -> str:
    ...
    try:
        ...
        if "available_categories" in result and len(
...
def get_browser_items_at_path(ctx: Context, path: str) -> str:
    ...
    try:
        ...
        if "error" in result and "available_categories" in result:
            ...
            available_cats = result.get("available_categories", [])
            return (
                f"Error: {error}\n" f"Available browser categories: {
                    ', '.join(available_cats)}")
...
def load_drum_kit(ctx: Context, track_index: int,
    ...
    try:
        ...
        loadable_kits = [
            item for item in kit_items if item.get(
                "is_loadable", False)]
        ...
        return f"Loaded drum rack and kit '{
            loadable_kits[0].get('name')}' on track {track_index}"
    except Exception as e:
...
@mcp.tool()
def get_device_parameters(ctx: Context, track_index: int,
...
@mcp.tool()
def set_device_parameter(ctx: Context,
                         track_index: int,
                         device_index: int,
...
def set_eq_band(ctx: Context,
                ...
                filter_type: Optional[Union[int,
                                            str]] = None) -> str:
    """
    ...
    try:
        ...
        if mode is not None:
            ...
            if mode_param:
                ...
                results.append(
                    "Warning: Could not find a Mode parameter in EQ Eight")

        # Set oversampling if provided - Note: EQ Eight doesn't seem to have an "Oversampling" parameter
        # We'll check if there's a parameter with "Oversampling" or "Hi
        # Quality" in its name
        if oversampling is not None:
            ...
            if oversampling_param:
                ...
                results.append(
                    f"Set {
                        oversampling_param['name']} to {
                        'enabled' if oversampling else 'disabled'}")
            else:
                results.append(
                    "Warning: Could not find an Oversampling parameter in EQ Eight")
...
@mcp.tool()
def apply_eq_preset(ctx: Context, track_index: int,
                    device_index: int, preset_type: str) -> str:
    """
    ...
    try:
        ...
        if preset_type not in presets:
            return f"Error: Unknown preset type '{preset_type}'. Available presets: {
                ', '.join(
                    presets.keys())}"
...
@mcp.tool()
def set_send_level(ctx: Context, track_index: int,
                   send_index: int, value: float) -> str:
    """
    ...
    try:
        ...
        return f"Set send level from track {
            result.get(
                'track_name',
                'unknown')} to {
            result.get(
...
def set_track_volume(ctx: Context, track_index: int, value: float) -> str:
    ...
    try:
        ...
        return f"Set volume of track {
            result.get(
                'track_name',
                'unknown')} to {volume_db_str}"
    except Exception as e:
