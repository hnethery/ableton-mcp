...
def install_remote_script(ableton_path: Optional[str] = None, force: bool = False) -> None:
    ...
    if not os.path.exists(source_path):
        print(
            f"Error: Remote Script source directory not found at {source_path}")
        sys.exit(1)
    ...
    print(f"Installing AbletonMCP Remote Script...")
    ...
    print(f"  ✓ Copied {files_count} files")
    if dirs_count > 0:
        print(f"  ✓ Copied {dirs_count} directories")

    print(f"\n✨ Successfully installed Remote Script to {target_path}")
    print("\n⚠️  IMPORTANT: You need to restart Ableton Live for the changes to take effect.")
...
def main() -> None:
    ...
    if args.command == "server":
        ...
        # Start the server
        print(
            f"Starting Ableton MCP server v{get_version()} on http://{args.host}:{args.port}")
